...
// parse craft formulas into cards
MOEPARSER.parseCraft = function (file) {
	file = file || "../data/craft.json";
	var cards = new Array();
	...
	for (var i in formulas) {
		...
		for (var j in formulas[i]) {
			if (typeof formulas[i][j] != "object" && typeof formulas[i][j] != "array") {
				continue;
			}
			// for each item
			for (var k in formulas[i][j]) {
				if (card_names.indexOf(formulas[i][j][k]) == -1 && j != "output") {
					card_names.push(formulas[i][j][k]);
					cards.push({
						name: formulas[i][j][k],
						type: "output" == j ? "hidden" : "",
						photo: ""
					});
				}
			}
	...
	// write them back
	fs.writeFile("data/craft1.json", JSON.stringify(cards, null, 4), function (err) {
		if (err) {
			console.error("file error", err);
		}
	});
